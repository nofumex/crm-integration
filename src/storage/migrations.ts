...
export async function validateSchema(pool:Pool):Promise<void>{const r=await pool.query("SELECT version FROM schema_migrations ORDER BY version DESC LIMIT 1");if(Number(r.rows[0]?.version)!==5)throw new Error("Database schema is not migrated; run npm run migrate");}
