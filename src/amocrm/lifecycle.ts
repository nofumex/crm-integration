...
export class AmoChatsLifecycle {
 ...
 async ensureSource(accountId:string,pipelineId?:number):Promise<void>{const account=await this.accounts.get(accountId);if(!account)throw new Error(`Unknown account ${accountId}`);if(account.messenger!=="telegram")throw new Error(`${account.messenger} Personal transport is unavailable; refusing to create a misleading amoCRM source`);const existing:any=await this.rest.findSources(account.sourceExternalId);if((existing?._embedded?.sources??[]).length)return;await this.rest.createSources([{name:account.displayName??`${account.messenger} ${account.providerAccountId}`,external_id:account.sourceExternalId,...(pipelineId?{pipeline_id:pipelineId}:{})}]);}
}
