...
interface StoredJob extends Job { lockedBy?: string; lockedAt?: Date; lastError?: string;createdAt:Date;updatedAt:Date;payloadPruned?:boolean }
...
export class InMemoryJobQueue implements JobQueue {
  ...
  async enqueue(input: NewJob): Promise<{ inserted: boolean; id: number }> {
    ...
    const job:StoredJob={...input,id:++this.sequence,state:"pending",attempts:0,maxAttempts:input.maxAttempts??12,availableAt:this.now(),createdAt:this.now(),updatedAt:this.now()};this.jobs.push(job);return{inserted:true,id:job.id};
  }
  ...
  async requeueDead(id:number,allowDeliveryUnknown=false){const j=this.jobs.find(x=>x.id===id&&x.state==="dead");if(!j||(!allowDeliveryUnknown&&j.lastError?.startsWith("DeliveryUnknownError:")))return false;j.state="pending";j.attempts=0;j.availableAt=this.now();j.lastError=undefined;return true;}
  async cleanup(r:{payloadBefore:Date;completedBefore:Date;deadBefore:Date}){let payloadsPruned=0,jobsDeleted=0;for(const j of this.jobs)if((j.state==="completed"||j.state==="dead")&&!j.payloadPruned&&j.updatedAt<r.payloadBefore){j.payload={};j.payloadPruned=true;payloadsPruned++;}for(let i=this.jobs.length-1;i>=0;i--){const j=this.jobs[i]!;if((j.state==="completed"&&j.updatedAt<r.completedBefore)||(j.state==="dead"&&j.updatedAt<r.deadBefore)){this.jobs.splice(i,1);jobsDeleted++;}}return{payloadsPruned,jobsDeleted};}
