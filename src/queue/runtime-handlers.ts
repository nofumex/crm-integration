...
export function createRuntimeHandlers(router:MessageRouter,adapters:AdapterRegistry):Record<"amocrm.outbound"|"messenger.inbound"|"messenger.status",JobHandler>{return{
 ...
 "messenger.inbound":async job=>{const p=job.payload as any;if(p.adapterWebhook){const adapter:any=required(adapters,p.messenger,p.accountId);if(p.messenger==="whatsapp")await adapter.acceptWebhook(p.body);else if(p.messenger==="max")await adapter.acceptUpdate(p.body,p.accountId);else throw new Error("Unsupported adapter webhook");return;}await router.routeInbound({...p,occurredAt:new Date(p.occurredAt),attachments:p.attachments??[]} as NormalizedMessage);},
