...
export class TelegramOnboardingService {
 ...
 constructor(private readonly secrets:SecretStore,private readonly accounts:AccountRepository,private readonly createAdapter:(input:{accountId:string;apiId:number;apiHash:string})=>AuthorizingAdapter=input=>new TelegramAdapter({...input,session:""}),private readonly logger?:{error(data:unknown,message?:string):void}){}
