...
export class TelegramOnboardingService {
 ...
 async start(input:StartInput):Promise<{accountId:string;status:"awaiting_code"}>{const accountId=input.accountId??randomUUID();return this.serial.run(accountId,()=>this.startLocked(input,accountId));}
 private async startLocked(input:StartInput,accountId:string):Promise<{accountId:string;status:"awaiting_code"}>{
  if(this.pending.has(accountId))throw new Error("Onboarding already active");
  ...
  const adapter=this.createAdapter({accountId,apiId:this.settings.apiId,apiHash:this.settings.apiHash});try{const delivery=await adapter.beginAuthorization(input.phone);if(!await this.accounts.get(accountId)){await adapter.disconnect();throw new Error("Telegram onboarding was cancelled");}this.pending.set(accountId,{adapter,credentialRef,delivery,awaitingPassword:false});return{accountId,status:"awaiting_code"};}catch(error){this.logAuthorizationError(error);await this.fail(accountId);throw new Error("Telegram authorization failed");}
 }
 async submitCode(accountId:string,code:string):Promise<{status:"awaiting_password"|"completed"}>{return this.serial.run(accountId,()=>this.submitCodeLocked(accountId,code));}
 private async submitCodeLocked(accountId:string,code:string):Promise<{status:"awaiting_password"|"completed"}>{const p=this.required(accountId);try{const result=await p.adapter.submitAuthorizationCode(code);if(result==="awaiting_password"){p.awaitingPassword=true;return{status:"awaiting_password"};}await this.complete(accountId,p);return{status:"completed"};}catch(error){this.logAuthorizationError(error);await this.fail(accountId);throw new Error("Telegram authorization failed");}}
 async submitPassword(accountId:string,password:string):Promise<{status:"completed"}>{return this.serial.run(accountId,()=>this.submitPasswordLocked(accountId,password));}
 private async submitPasswordLocked(accountId:string,password:string):Promise<{status:"completed"}>{const p=this.required(accountId);if(!p.awaitingPassword)throw new Error("2FA password is not requested");try{await p.adapter.submitAuthorizationPassword(password);await this.complete(accountId,p);return{status:"completed"};}catch(error){this.logAuthorizationError(error);await this.fail(accountId);throw new Error("Telegram authorization failed");}}
 async resendCode(accountId:string):Promise<TelegramCodeDelivery>{return this.serial.run(accountId,()=>this.resendCodeLocked(accountId));}
 private async resendCodeLocked(accountId:string):Promise<TelegramCodeDelivery>{const p=this.required(accountId);try{p.delivery=await p.adapter.resendAuthorizationCode();return p.delivery;}catch(error){this.logAuthorizationError(error);throw new Error("Telegram could not resend the authorization code");}}
 ...
 async cancel(accountId:string):Promise<void>{await this.serial.run(accountId,()=>this.cancelLocked(accountId));}
 async withAccountLock<T>(accountId:string,task:()=>Promise<T>):Promise<T>{return this.serial.run(accountId,task);}
 async cancelLocked(accountId:string):Promise<void>{const p=this.pending.get(accountId);if(!p)return;this.pending.delete(accountId);await p.adapter.disconnect();}
 ...
 private async fail(accountId:string){this.pending.delete(accountId);if(await this.accounts.get(accountId))await this.accounts.setState(accountId,"reconnect_required","Telegram authorization failed");}
