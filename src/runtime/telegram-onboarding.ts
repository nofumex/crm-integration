...
export class TelegramOnboardingService {
 ...
 async resendCode(accountId:string):Promise<TelegramCodeDelivery>{const p=this.required(accountId);try{const delivery=await p.adapter.resendAuthorizationCode();this.active(accountId,p);p.delivery=delivery;return p.delivery;}catch(error){this.logAuthorizationError(error);throw new Error("Telegram could not resend the authorization code");}}
 ...
 async cancel(accountId:string):Promise<void>{const p=this.pending.get(accountId);if(!p)return;this.pending.delete(accountId);await p.adapter.disconnect();}
 private required(id:string){const p=this.pending.get(id);if(!p)throw new Error("No active onboarding for account");return p;}
 private active(accountId:string,p:Pending){if(this.pending.get(accountId)!==p)throw new Error("Telegram onboarding was cancelled");}
 private async complete(accountId:string,p:Pending){this.active(accountId,p);await this.secrets.put(p.credentialRef,{session:p.adapter.authorizationSession(),authorized:true});this.active(accountId,p);await this.accounts.setState(accountId,"disconnected");await p.adapter.disconnect();this.pending.delete(accountId);}
