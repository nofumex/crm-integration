...
describe("startup safety validation",()=>{
  ...
  it("selects a fully configured private OAuth client without replacing legacy settings",()=>{const c=loadConfig({...valid,AMOCRM_OAUTH_PROFILE:"private",AMOCRM_PRIVATE_INTEGRATION_ID:"private-id",AMOCRM_PRIVATE_CLIENT_SECRET:"private-secret",AMOCRM_PRIVATE_REDIRECT_URI:"https://sinaichannel.ru/oauth/callback"} as any);expect(c.AMOCRM_OAUTH_PROFILE).toBe("private");expect(c.AMOCRM_PRIVATE_INTEGRATION_ID).toBe("private-id");});
  it("refuses a private OAuth switch without its complete client configuration",()=>{expect(()=>loadConfig({...valid,AMOCRM_OAUTH_PROFILE:"private"} as any)).toThrow("private OAuth profile");});
  it("requires a verified target and Chats origin code before enabling source writes",()=>{expect(()=>loadConfig({...valid,AMOCRM_SOURCES_WRITES_ENABLED:"true"} as any)).toThrow("source writes require");});
});
