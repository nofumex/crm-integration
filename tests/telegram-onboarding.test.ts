...
describe("Telegram onboarding recovery",()=>{
 it("creates the placeholder secret before a new account row to satisfy credential_ref_fkey",async()=>{const accounts=new InMemoryAccountRepository(),secrets=new InMemorySecretStore();const original=accounts.upsert.bind(accounts);const upsert=vi.spyOn(accounts,"upsert").mockImplementation(async account=>{expect(await secrets.get(account.credentialRef)).toEqual({session:"",authorized:false});await original(account);});const service=new TelegramOnboardingService(secrets,accounts,{apiId:42,apiHash:"global-hash"},()=>({beginAuthorization:async()=>({method:"app",canResend:false} as const),submitAuthorizationCode:vi.fn(),submitAuthorizationPassword:vi.fn(),resendAuthorizationCode:vi.fn(),authorizationSession:()=>"",disconnect:vi.fn()}));const started=await service.start({phone:"+79990000001",amoAccountId:"17354872"});expect(upsert).toHaveBeenCalledOnce();expect(await accounts.get(started.accountId)).toMatchObject({credentialRef:`telegram:${started.accountId}`,sourceExternalId:`tg-${started.accountId.replace(/-/g,"")}`});expect(await secrets.get(`telegram:${started.accountId}`)).toEqual({session:"",authorized:false});});
 it("removes the placeholder secret when creating the account row fails",async()=>{const secrets=new InMemorySecretStore();let ref="";const accounts={get:async()=>undefined,upsert:async(account:any)=>{ref=account.credentialRef;throw new Error("credential_ref_fkey");}} as any;const service=new TelegramOnboardingService(secrets,accounts,{apiId:42,apiHash:"global-hash"},()=>{throw new Error("should not create adapter");});await expect(service.start({phone:"+79990000002",amoAccountId:"17354872"})).rejects.toThrow("credential_ref_fkey");expect(await secrets.get(ref)).toBeUndefined();});
 it("reauthorizes an existing account in place and replaces its credential secret",async()=>{
  ...
  await secrets.put(`telegram:${id}`,{session:""});
  const disconnect=vi.fn(async()=>undefined);
  const service=new TelegramOnboardingService(secrets,accounts,{apiId:42,apiHash:"global-hash"},()=>({beginAuthorization:vi.fn(async()=>({method:"app",nextMethod:"sms",canResend:true} as const)),submitAuthorizationCode:vi.fn(async()=>"completed" as const),submitAuthorizationPassword:vi.fn(),resendAuthorizationCode:vi.fn(async()=>({method:"sms",canResend:false} as const)),authorizationSession:()=>"authorized-session",disconnect}));
  const started=await service.start({accountId:id,phone:"+79990000000",amoAccountId:"ignored"});
  ...
  expect(await accounts.listAll()).toHaveLength(1);expect(await accounts.get(id)).toMatchObject({credentialRef:`telegram:${id}`,state:"disconnected"});
  expect(await secrets.get(`telegram:${id}`)).toEqual({session:"authorized-session",authorized:true});expect(disconnect).toHaveBeenCalledOnce();
 });
