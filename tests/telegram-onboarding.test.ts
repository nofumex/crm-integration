...
describe("Telegram onboarding recovery",()=>{
 it("reauthorizes an existing account in place and replaces its credential secret",async()=>{
  ...
  const service=new TelegramOnboardingService(secrets,accounts,{apiId:42,apiHash:"global-hash",sourceExternalId:"global-source"},()=>({beginAuthorization:vi.fn(async()=>({method:"app",nextMethod:"sms",canResend:true} as const)),submitAuthorizationCode:vi.fn(async()=>"completed" as const),submitAuthorizationPassword:vi.fn(),resendAuthorizationCode:vi.fn(async()=>({method:"sms",canResend:false} as const)),authorizationSession:()=>"authorized-session",disconnect}));
