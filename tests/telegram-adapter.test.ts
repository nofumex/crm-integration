...
describe("Telegram MTProto adapter", () => {
  ...
  it("selects the PeerUser match when ResolvePhone returns multiple users",async()=>{const first={id:41,accessHash:"first"},selected={id:42,accessHash:"selected"};const fake=phoneResolverFake({peer:{className:"PeerUser",userId:42},users:[first,selected]});const adapter=new TelegramAdapter({accountId:"tg-1",apiId:1,apiHash:"hash",session:"session",client:fake as any});await expect(adapter.resolveRecipient({phone:"+79991234567"})).resolves.toMatchObject({providerRecipientId:"42",providerRecipientRef:{userId:"42",accessHash:"selected"}});expect(fake.getInputEntity).toHaveBeenCalledWith(selected);});
