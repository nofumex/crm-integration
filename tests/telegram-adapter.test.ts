...
describe("Telegram MTProto adapter", () => {
  it("resolves a persisted Telegram InputPeer before sending instead of passing a raw numeric ID", async () => {
    ...
    expect(inbound).toHaveBeenCalledWith(expect.objectContaining({accountId:"tg-1",conversationId:"77123",id:"8",sender:expect.objectContaining({externalId:"77123",displayName:"Ada Lovelace",username:"ada",phone:"+79990000000",profile:{firstName:"Ada",lastName:"Lovelace",username:"ada",phone:"+79990000000",isBot:false}})}));
  });
  ...
  it("uses username or numeric ID as a safe display name when Telegram omits names", async () => {
    ...
    expect(inbound.mock.calls[0]![0].sender).toMatchObject({externalId:"10",displayName:"only_username",username:"only_username",profile:{username:"only_username"}});
    expect(inbound.mock.calls[1]![0].sender).toMatchObject({externalId:"11",displayName:"11"});
  });
  ...
  it("returns a safe permanent error when an uncached numeric recipient cannot be resolved", async () => {
    ...
    await expect(adapter.send({accountId:"tg-1",conversationId:"7727079839",recipientId:"7727079839",text:"hello",idempotencyKey:"amo"})).rejects.toThrow("Telegram recipient is unavailable");expect(fake.sendMessage).not.toHaveBeenCalled();
  });
});
